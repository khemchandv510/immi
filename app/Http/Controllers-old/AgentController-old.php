...
//   dd($request->id, $request->agent);

    $agent = DB::table('users')->where('unique_id',$request->agent)->get();
    

    $get_client = DB::table('enquiries')->where('unique_id',$request->a)->get();
    


    $enq_asign_clients = new  enq_asign_clients([
        'unique_id'  =>  $unique_id = mt_rand(11111111,99999999),
        'from_agent_unique_id' => $get_client[0]->agent_unique_id ,
        'from_agent_id' =>  $get_client[0]->agent_id,
        'from_agent_name' =>  $get_client[0]->agent_name, 
        'client_enquiry_id' => $get_client[0]->unique_id,
      
        'comment'  =>$request->asign_comment,
        'client_name'        => $get_client[0]->name,
  ...
  $client->where('unique_id',$request->a)
            ->update([
...
public function asign_client_list(){
    return view('enquiry/asign-client-list');
}
