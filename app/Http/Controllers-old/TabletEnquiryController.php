...
if($find_agent[0]->usertype_status == 1){
    ...
    $get  = DB::table('enquiries')->where('conversion',0)->where('delete_client','1')->orderBy('id', 'DESC')->paginate(15);
    
}
else{
$get  = DB::table('enquiries')->where('conversion',0)->where('agent_id', $id)->where('delete_client','1')->orderBy('id', 'DESC')->paginate(15);
...
public function get_all_priority(Request $request){
    ...
    $priority = DB::table('enquiries')->where('delete_client',1)->where('set_priority',$request->id)->where('agent_unique_id',Auth::user()->unique_id)->get();
    }
    // dd($priority);
