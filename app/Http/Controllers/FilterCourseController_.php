...
$where .= "    courses.status=1 ";
    if(!isset($offset)){
        ...
        $filterCourses  = DB::select(DB::raw("SELECT *,courses.unique_id as unique_id,courses.course_name as course_name,courses.duration_year as duration_year,courses.intake as intake,courses.application_fee as application_fee,courses.fees as fees, courses.course_url as courseurl ,my_institutes.name as `name`, my_institutes.logo as `image`, my_institutes.website as `website`, my_institutes.code as code, my_institutes.country as country, my_institutes.city as city, courses.min_education_eligibility as  min_education_eligibility from courses INNER JOIN my_institutes ON courses.college_code = my_institutes.code $where limit 10"));
return array($filterCourses,$filterCoursesCount);
    }
        else{
            $filterCourses  = DB::select(DB::raw("SELECT *,courses.unique_id as unique_id,courses.course_name as course_name,courses.duration_year as duration_year,courses.intake as intake,courses.application_fee as application_fee,courses.fees as fees,my_institutes.name as `name`, courses.course_url as courseurl ,my_institutes.code as code, my_institutes.country as country, my_institutes.city as city, my_institutes.logo as `image`, my_institutes.website as `website`, courses.min_education_eligibility as  min_education_eligibility  from courses INNER JOIN my_institutes ON courses.college_code = my_institutes.code $where limit $limit offset $offset "));
