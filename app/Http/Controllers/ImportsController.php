...

{
    ...
    // Public function deletecanada(){
    //     $institutes = DB::table('my_institutes')
    //     // ->select('code')
...
// dd($college_code, $importData[16]),;
    ...
    // dd($importData);
     ...
     // "college_code"  =>$college_code,
        ...
        'duration_year' => $importData[28],
        'city'            => $importData[29],
        'language'        => $importData[30],
        'other_requirments' => $importData[31],
        'course_url'   => $importData[32],
        'campus'       => $importData[33],
        'application'  => $importData[34],
        'Scholarship'  => $importData[35],
        'Scholarship_details' => $importData[36],
        'Backlog_Range' => $importData[37],
        'Remarks' => $importData[38],
        'ESL_ELP_Detail' => $importData[39],
        'ApplicationMode' => $importData[40],
        'DetScore' => $importData[41],
        'GRE'  => $importData[42],
        'GMAT' => $importData[43],
        // 'count' => $importData[44]
        // 'per'              => $importData[28],
        ...
        if($request->hasfile('image'))
         {
            foreach($request->file('image') as $file)
            {
                ...
                $files[] = $name;  
                $file->move(public_path('uploads/university-logo/'), $name);  
            }
            return back();
         }else{
             return back();
...
//   $comma = (str_replace("\\","","$importData[6]"));
...
//   dd($columns); 
  ...
  $value = 'city';
  $replace = 'my_institutes.city';
  
  
  
$columns=   array_map(function ($v) use ($value, $replace) {
    return $v == $value ? $replace : $v;
}, $columns);


//   $a = array_map(function($columns) use ($value, $replace){
//       if(in_array('city',$columns)){
//      return $columns = $value ?  $replace : $columns;
//       }
//   }, $columns);
...
public function import_hot_leads(Request $request){
