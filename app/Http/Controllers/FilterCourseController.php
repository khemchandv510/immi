...
//  dd($request->student_unique_id);
    ...
    enquiries::where('unique_id',$request->student_unique_id[0])
    ->update(['payment_done'=>0,
    'payment_pending_date' => date('Y-m-d') ,
   ...
   Session::flash('message','Courses Sort Listed');
    ...
    return redirect("admin/enquiry/get-detail/".$request->student_unique_id[0]);
    }
    elseif(Auth::user()->usertype_status == 4){
        return redirect("employee/enquiry/get-detail/".$request->student_unique_id[0]);
    }
    elseif(Auth::user()->usertype_status == 5){
        return redirect("subagent/enquiry/get-detail/".$request->student_unique_id[0]);
    }
    elseif(Auth::user()->usertype_status == 6){
        return redirect("icfei/enquiry/get-detail/".$request->student_unique_id[0]);
    }
...
function filter_course_section(Request $request)
{
   ...
   enquiries::where('unique_id',$id)->update(['search_course'=>1]);
    return view('filter-course',compact(['id','campus']));
}
...
// echo $filter_university_list;

if(isset($request->search) && (!empty($request->search))){
    // $college_code = join("','", $request->college_code);    
    $search =$request->search;
    $where .= " ( courses.course_name like ('%".$search."%') or my_institutes.state like ('%".$search."%') or my_institutes.city like ('%".$search."%') or my_institutes.country like ('%".$search."%') ) and ";
}
else{
    $search = "";
}

if(isset($request->campus) && (!empty($request->campus))){
    
    $campus = join("','", $request->campus);     
    
    $where .= " courses.campus IN ('".$campus."') and ";
}
else{
    $campus = "";
}
...
if($request->limit !="" ){
$offset = (int)$request->limit*((int)$request->page-1);
// dd($offset, (int)$request->offset);
}else{
    $offset = 0;
}
$page = 1;
...
// dd($where, $limit, $offset );
    if(isset($offset)){
        
        $filterCourses  = DB::select(DB::raw("SELECT *,courses.unique_id as unique_id,courses.course_name as course_name,courses.duration_year as duration_year,courses.intake as intake,courses.application_fee as application_fee,courses.fees as fees,my_institutes.name as `name`, my_institutes.logo as `image`,my_institutes.code as code, my_institutes.country as country, my_institutes.city as city, courses.min_education_eligibility as  min_education_eligibility, my_institutes.website as website, courses.course_url from courses INNER JOIN my_institutes ON courses.college_code = my_institutes.code $where limit $limit offset $offset "));
        
// dd($filterCourses, $offset, count($filterCoursesCount), $limit);

// dd($offset, $request->page, $request->limit);



        

        $total_data =   count($filterCoursesCount);
// dd($filterCourses,$filterCoursesCount,$total_data, $pagination_html);
if($total_data < 1){
    $pagination_html = [];
  return [$filterCourses,$filterCoursesCount,$total_data, $pagination_html];
}
// dd($filterCourses,$filterCoursesCount,$total_data);
        $_POST["query"] = '';
...
return array($filterCourses,$filterCoursesCount,$total_data, $pagination_html);
    }
        else{
...
// dd($total_data,$limit);
        ...
        <div align="center">
              <ul class="pagination">
        ';
    // dd($filterCoursesCount,$limit);
        $total_links = ceil($total_data/(int)$limit);
...
if(isset($request->search) && (!empty($request->search))){
   ...
   $where .= " ( courses.course_name like ('%".$search."%') or my_institutes.state like ('%".$search."%') or my_institutes.city like ('%".$search."%') or my_institutes.country like ('%".$search."%') ) and ";
    ...
    // $where .= "  courses.course_name like ('%".$search."%')  and ";
}
else{
    $search = "";
}
...
public function update_priority(Request $request){
    ...
    $check  = sort_list_courses::where('student_unique_id',$request->unique_id)
    ->where('priority', '>=', $request->val)
    ->where('course_unique_id', '!=', $request->course_id)
    ...
    $prtyval = $request->val;
    if($check->count() > 0){
        foreach($check as $updatePty){
            $prtyval++;
            
            sort_list_courses::where('student_unique_id',$updatePty->student_unique_id)
            ->where('course_unique_id',$updatePty->course_unique_id)
            ->update(['priority'=>$prtyval]);
        }
    }
    
    // if($check->count() > 0){
    //   echo "This Priority Already Exist!";  
    // }else{
