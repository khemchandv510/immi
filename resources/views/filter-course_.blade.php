...
<?php 
        // $university = DB::select(DB::raw("SELECT * from my_institutes  limit 50  "));
...
$filterCoursesLoad  = DB::select(DB::raw("SELECT courses.unique_id as unique_id,courses.course_name as course_name,courses.duration_year as duration_year,courses.intake as intake,courses.application_fee as application_fee,courses.fees as fees,my_institutes.name as `name`, my_institutes.image as `image`,my_institutes.code as code, my_institutes.country as country, my_institutes.city as city,my_institutes.website as website , courses.min_education_eligibility as  min_education_eligibility from courses INNER JOIN my_institutes ON courses.college_code = my_institutes.code  limit 10"));

// dd($filterCoursesLoad);
...
{{-- <div class="college-detail-page filter-eligibility">
  <div class="eligibility" >
    <h5>Eligibility</h5>
      
      <table class="min-qualification">
        <tbody><tr>
          <td> <strong> Min Qualification  </strong>  </td> <td> Grade 12 / High School </td>
        </tr>
        <tr>
          <td> <strong> Min GPA </strong>  </td>  <td>  </td>
        </tr>
      </tbody></table>
...
<script>
 ...
 $(document).ready(function(){
   ...
   $(document).on('click',"#selectedOptionDiv .fa-close",(event)=>{
    ...
    $('#country,#state, #city, #university, #batcheloreMaster, #duration, #stream, #substream, #intake, #tutionFees, .get-college-course, #priority_university').change(function(event){  
...
data:{country:country,state:state,city:city,university:university,batcheloreMaster:batcheloreMaster,duration:duration,stream:stream,substream:substream,intake:intake,tutionFees:tutionFees,college_code:college_code, priority_university:priority_university, filter_university_list:filter_university_list},
success: function(data){
    // console.log(data[0]);
    $('.parent-filter-courses ul ').empty();
    $('.demo').hide();
...
var ielts_set      = 'IELTS '+data[0][i].ielts_speaking+' Overall (none than '+data[0][i].min_ilets_overall+' bands),'
}
else{ 
  var ielts_set_spcl ="";
  var ielts_set ="";
...
if(data[0][i].toefl_reading != null){
  var toefl_set =  'TOEFL '+data[0][i].toefl_reading+' Overall (none less than '+data[0][i].min_toefl_ibt+'),'  ;
}else{
  var toefl_set ='';
}
if(data[0][i].min_pte_reading != null){
  var pte_set          = 'PTE '+data[0][i].min_pte_reading+' Overall (none less than '+data[0][i].min_pte_overall+')';  
}else{
  var pte_set = '';
}
...
//     let prev = x[i-1].college_code;
//     let current = x[i].college_code;
//     if(prev != current){
//        y.push(x[i]);
//        console.log(y);
//      }
...
//     $('.filter-university-list').append(` <li style="position:relative"> <input type="checkbox"  value=" `+y[i].college_code+`"><span> `+y[i].college_code+` </span> <input type="text" class="course-count" disabled value="10"> </li>`); 
...
data:{university:university,city:city,batcheloreMaster:batcheloreMaster,duration:duration,stream:stream,substream:substream,intake:intake,tutionFees:tutionFees,priority_university:priority_university,country:country},
success: function(data){
  $('#university').empty();
var len = data.length;
$('.filter-university-list').empty();
...
$('.filter-university-list').append(`<li style="position:relative;list-style: none;    border: 1px solid #e2d4d4;    width: 100%;    margin: 0;
    padding: 5px 3px;" > <input type="radio"  value="`+data[i].college_code+`" class="get-college-course" style="width: 21px;" name="dasdas" id="a${data[i].college_code}"><label style="cursor:pointer; display: contents;" for="a${data[i].college_code}"> `+data[i].name+` </span>
     </li>`);}
...
// $('input[name="college_code"]').on('change', function(){
    // $('.demo').show();
    // var id =  $(this).val();
// alert();
//     if(id.length > 0){
...
data:{country:country,state:state,city:city,university:university,batcheloreMaster:batcheloreMaster,duration:duration,stream:stream,substream:substream,intake:intake,tutionFees:tutionFees,limit:limit,offset:offset},
success:function(data){
var len = data.length;
...
var ielts_set      = 'IELTS '+data[i].ielts_speaking+' Overall (none than '+data[i].min_ilets_overall+' bands),'
}
else{ 
  var ielts_set_spcl ="";
  var ielts_set ="";
...
if(data[i].toefl_reading != null){
  var toefl_set =  'TOEFL '+data[i].toefl_reading+' Overall (none less than '+data[i].min_toefl_ibt+'),'  ;
}else{
  var toefl_set ='';
}
if(data[i].min_pte_reading != null){
  var pte_set          = 'PTE '+data[i].min_pte_reading+' Overall (none less than '+data[i].min_pte_overall+')';  
}else{
  var pte_set = '';
}
...
<div class="my-course-list-parent" style="width:0px ">
        <ol id="my_course_List" >
          ...
          <!--<button type="button"  class="btn btn-danger w-10 mt-1" id="download_corse_whatsapp_button" data-toggle="modal" data-target="#download_send_corse_whatsapp">Download pdf </button>-->
         
          <button type="button"  class="btn btn-danger w-10 mt-1" id="send_corse_whatsapp_button" data-toggle="modal" data-target="#send_corse_whatsapp">Whatsapp</button>
...
// for whatsapp 
     ...
     $("#send_corse_whatsapp_button").click(function(){
     let count = 0;
     let course_id = [];
        $("#my_course_List li").each(function(){
        ...
        $(".course_whatsapp_ids").val(course_ids);
        // alert(course_ids);
});

//  download pdf for whatsapp
// $("#download_corse_whatsapp_button").click(function(){
//      let count = 0;
//      let course_id = [];
//         $("#my_course_List li").each(function(){
...
<button class="btn btn-danger w-10 mybtn" type="submit"> Send Mail</button>
      ...
      <div class="modal" id="send_corse_whatsapp" style="z-index:100">
        <div class="modal-content">    
          ...
          <!-- Modal body -->
          <div class="container my-3" id="">
              <form method="POST" action="{{ url(Request::segment(1).'/send-whatsapp-pdf-download') }}" target="_blank">    
          @CSRF      
            <input type="hidden" id="" name="courses_ids" class="course_whatsapp_ids">
            <input type="hidden" value="https://hms.craftzvilla.com/client-manager/public/images/logo-color.png" name="text">
            
            <!--<input type="text" class="form-control" name="phone" required>-->
              <button class="btn btn-danger w-10 mybtn" type="submit"> Download pdf </button>
          </form>
          <h2 class="text-center"> OR Send by Whatsapp</h2>
          <form method="POST" action="{{ url(Request::segment(1).'/send-whatsapp-content') }}" target="_blank">    
          @CSRF      
            <input type="hidden" id="" class="course_whatsapp_ids" name="courses_ids">
            <input type="hidden" value="https://hms.craftzvilla.com/client-manager/public/images/logo-color.png" name="text">
            
            <input type="text" class="form-control" name="phone" placeholder="Enter whatsapp number" required>
...
<!--ens emailer modal-->
    ...
    <!--<div class="modal" id="download_send_corse_whatsapp" style="z-index:100">-->
    <!--    <div class="modal-content">    -->
          <!-- Modal Header -->
    <!--      <div class="modal-header">-->
    <!--        <h4 class="modal-title">Download pdf  Whatsapp</h4>-->
    <!--        <button type="button" class="close removecourese" data-dismiss="modal">&times;</button>-->
    <!--      </div>-->
    ...
    <!--      <form method="POST" action="{{ url(Request::segment(1).'/send-whatsapp-content') }}" target="_blank">    -->
    <!--      @CSRF      -->
    <!--        <input type="hidden" id="course_whatsapp_ids" name="courses_ids">-->
    <!--        <input type="hidden" value="https://hms.craftzvilla.com/client-manager/public/images/logo-color.png" name="text">-->
