          ...
          <div class="row list-page-top-link" >
            ...
            <div class="col-md-12"> 
              ...
              <ul class="enquiry-test-list">
                <a href="{{route('add-leads')}}" class="collapse-item "><li>
               ...
               $trash = DB::table('enquiries')->where('delete_client','0')->get();
                ?>  
                <a href={{ url('get-all-trash-clients') }}><li><i class="far fa-arrow-alt-circle-right"></i> {{$trash->count()}} Trashed  </li> </a>
                <?php } ?>
                {{-- <li><a href={{ url('get-all-list/'.Crypt::encrypt(Auth::user()->employee_id)) }}><i class="fas fa-angle-double-right"></i>  Get All  </a></li> --}}
                <?php 
            if( Auth::user()->usertype_status =='1'){
              ?> <a href={{ url('asign-client-list') }}><li><i class="fas fa-user-tie"></i> Asigned Client  </li></a> <?php } ?>
                ...
                <a  data-toggle="modal" data-target="#import_unfollow_client" href =""  aria-hidden="true" style="color:#fff"><li></i> <i class="fas fa-file-import"></i> Import Unfollow Client
                </li></a>
                
                <li>
                  <i class="fas fa-list-ul"></i> <span  class="priority-list-ul" style="padding:8px;">Priority List
                        <div class="hide-priority-list-ul">
                               <a href={{ url('priority-list/1/0') }} value="1">   High  </a> 
                               <a href={{ url('priority-list/2/0') }} value="2">   Medium  </a> 
                               <a href={{ url('priority-list/3/0') }} value="3">   Low  </a> 
                              </div>
                                     </span>
                </li>
              <li>{{-- 03-11-2020 --}}
             
                  <a href="" class="collapse-item " data-toggle="modal" data-target="#add_new_followup_client"><i class="fas fa-phone-alt"></i> Follow Up  </a>
...
<hr>
   {{ Form::open(array('url'=>'view-new-leads', 'class' => 'enquiry-agent-name', "method" =>"get" )) }}
 <input type="hidden" value="{{$new_leads}}" name="new_leads">
...
@endif 
            ...
            <div class="col-md-12" style=" text-align: center; ">
               ...
               <!--   {{ Form::submit('Reset', array('url'=>'enquiry-list', 'class' => 'enquiry-agent-name btn btn-danger w-10')) }} -->
                ...
                <a class="btn btn-danger w-10" href="{{url('enquiry-list/'.Crypt::encrypt(Auth::user()->employee_id)) }}">Reset</a>
...
<!--</select>-->
    ...
    <!--  <span data-href="/tasks" id="export" class="btn btn-success btn-sm" onclick="exportTasks(event.target);">Export</span> --> 
          ...
          <li class="btn-special btn-icon btn-googleplus" id="send-sms"><i class="fa fa-comment" aria-hidden="true" data-toggle="modal" data-target="#send-sms"></i><span style="padding-left: 5px; padding-right: 5px;" data-toggle="modal" data-target="#send-sms">Send sms</span></li>

          <li onclick ="" class="btn-special btn-icon btn-googleplus" id="send-sms"><i class="fa fa-comment" aria-hidden="true" data-toggle="modal" data-target=""></i><span style="padding-left: 5px; padding-right: 5px;" data-toggle="modal" data-target="#asign_client" class="">Assign</span></li>
      ...
      <center> <h2 class="text-primary">{{$new_leads}}</h2></center>
...
{{Form::open(array('url'=>'export', 'method'=>'get','id'=>'form_export_excell'))}}
...
<!--<a href="tel:123-456-7890">123-456-7890</a>-->
 ...
 <td>
                        ...
                        <select name="conversion" id="conversion" class="form-control  update_status" onchane="update_status(this.value);" data-id="21418423">
                      ...
                      <td>  <a title="Delete" href="javascript:void(0);" class="fa fa-trash delete"   data-id={{$s->unique_id}} ></a>   <a href={{'enquiry/get-detail/'.$s->unique_id}}><i class="fas fa-external-link-alt" aria-hidden="true"></i>
                      </a></td>
                </tr>
...
@endif
                  ...
                  <?php } ?>
                    ...
                    <td>  <a title="Delete" href="javascript:void(0);" class="fa fa-trash  delete"   data-id={{$get2->unique_id}} ></a>
                  @if( Auth::user()->usertype_status =='1')
 ...
 {{-- <label class="container-radio">
                    ...
                    <td>
                      ...
                      <td class="client-name"> 
                        ...
                        @if(Auth::user()->usertype_status == 1 || Auth::user()->usertype_status == 2 || Auth::user()->usertype_status == 3)  
                          <a style="color:#575757;" href={{url('enquiry/get-detail/'.$get2->unique_id)}}>  {{$get2->name}} </a>  
                        @else
                      ...
                      {{-- <td>{{$get2->email}}</td> --}}
                      <td class="mob-no">
                       <p><a href="tel:{{$get2->contact}}"style="color: black;">{{$get2->contact}}</a> 
...
@if(app('request')->input('type') == 0)
                      <td class="aa">
                        <select class="form-control  update_status" name="conversion" id="conversion">
                          <option value="">Select Conversion</option>
                          <option value="1">Student</option>
                          <option value="2">PR/Immigration</option>
                          <option value="3">Institute</option>
...
@endif
                    ...
                    @if( Auth::user()->usertype_status =='1')
                        {{-- <a href="{{url('enquiry/get-detail/'.$get2->unique_id)}}"  class="fa fa-edit" > --}}
                          <a title="Edit" href="{{url('edit-lead/'.$get2->unique_id)}}" class="fa fa-edit ">  </a>
                    ...
                    <tr><td colspan="13"><img src="{{url('public/uploads/NoRecordFound.png')}}" alt="" style="width:400px;height:400px"></td></tr>
                    @endif
                    {{-- @if(!empty($new_user)) --}}
  ...
  {{-- <tr><td colspan="14" style="hover:background:red"> 
    ...
    <tr><td colspan="14" style="hover:background:red">
    {{ $get->appends(['type'=>isset($_GET['type'])?$_GET['type']:'','filter_date_from'=>isset($filter_date_from)?$filter_date_from:"",'filter_date_to'=>isset($filter_date_to)?$filter_date_to:"",'agent_id'=>isset($agent_id)?$agent_id:"",'status'=>isset($status)?$status:"",'purpose_of_query'=>isset($purpose_of_query)?$purpose_of_query:"",'searchbox'=>isset($searchbox)?$searchbox:"",'range_filter'=>isset($range_filter)?$range_filter:"",'shortby'=>isset($shortby)?$shortby:"", 'new_leads'=>isset($new_leads)?$new_leads:""])->links() }} 
  </td></tr>
...
<script>
 ...
 $(document).ready(function(){
                 ...
                 e.preventDefault();
              var form = $(this); 
                var url = "{{url(Request::segment(1).'/callbacklater')}}?a="+unique_id+"&comment="+comment+"&val="+val;
                        ...
                        <div class="modal-dialog">
                          <div class="modal-content">
                            ...
                            <div class="modal-body">
                              ...
                              <form method='post' action={{url('import-unfollow-client')}} enctype='multipart/form-data' >
                                {{ csrf_field() }}
                                <div class="custom-file">
                                  <input type="file" class="custom-file-inpu" id="customFil" name="import_client">
...
document.getElementById('old_coment').innerHTML ="";
  ...
  // var cnt= 0;
  // document.querySelectorAll('.add_comment').forEach((a=>{
  //   a.addEventListener("click",function(){
  //     alert('dfd');
  //     document.getElementsByClassName('old-comment')[cnt].scrollTop = 10000;
  //     })
  //     cnt++;
  // }
  // ))
...
<script type="text/javascript">
   document.querySelectorAll('#conversion').forEach(element => {
    ...
    element.addEventListener('change',function(){
      ...
      var id = element.parentElement.parentElement.firstElementChild.nextElementSibling.nextElementSibling.dataset;
...
var sub =  document.querySelector('#send_mail');
sub.onclick = function(){
  var len = document.querySelectorAll('#form_export_excell table input[type="checkbox"]:checked').length
  if(len>0){
    var sub_array = [];
