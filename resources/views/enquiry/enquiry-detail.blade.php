...
if(validateDate($date, $formate = 'Y-m-d') == 1){
?>
<td> {{date('d/m/Y', strtotime($get->dob))}} </td>
<?php
}else{
?>
<td>  </td>
...
<div class="tab-content content-style    get-education-dynamic my-4" id="">
    ...
    <div role="tabpane" class="  tab-pane class{{$q->id}}" id="">
        <div style="text-align: right">
             <button data-toggle="modal" data-target=".education_edit{{$q->id}}" class="edit-text edit-button-design">Edit</button>
               {{Form::open(array('url'=>'delete-qualification', "onsubmit" => "return confirm('Are you sure to delete $q->class ?')",  "method"=>"get", 'style'=>"display:contents"))}}
                <input type="hidden" name="name_of_class" value="{{$q->class}}">
                <input type="hidden" value="{{session()->get('unique_id_sess')}}" name="unique_id">            
        <button type="submit" class="delete-text delete-button-design">Delete</button>
    {{Form::close()}}
        </div>


        {{-- <div style="text-align: right">
            <button class="edit-button-design" data-toggle="modal" data-target="#work_experience_edit3055">Edit</button>           
            <form method="POST" action="http://localhost/imm/imm/delete-experience-company" accept-charset="UTF-8" onsubmit="return confirm('Are you sure to delete  TCS ?')" style="display:initial"><input name="_token" type="hidden" value="ZKRrE3S6foLoyf2c5lD5fVTqoeqZmSX9HL5CsdLA">
            
          <input type="hidden" name="company_name" value="TCS">
          <input type="hidden" name="unique_id" value="99559097">
...
<div class="">
    ...
    <tr> <th>Country of Study </th> 
    
<?php 
  $country_namee =  DB::table('countries')->select('country_name')->where('country_id', '=', $q->country_of_study)->first();
if(!empty($country_namee->country_name)){
    $county_name = $country_namee->country_name;
}else{
    $county_name = "";
}
    ?>
    ...
    <td style="display: none;">
    <input type="text" name="country_of_study" value="{{$q->country_of_study}}" class="form-control "> </td></tr>
    ...
    <tr>
         ...
         <?php 
              $state_name =  DB::table('states')->select('state_name')->where('state_id', '=', $q->state_of_study)->first();
            ...
            if(!empty($state_name->state_name)){
                ...
                ?>
    <td >{{$stateName}}</td><td style="display: none;"> <input type="text" name="state_of_study" value="{{$q->state_of_study}}" class="form-control "> </td></tr>
...
<table>
    ...
    <tr><td>
        {!! $f->financials_by!!}{{($f->other?', '.$f->other:'') }}        </td>
<td> {!! $f->amount !!}</td>
<td>
...
{{-- eight tab --}}
@include('enquiry.country_before_travell')
...
<?php 
$get_finace_detail = DB::table('enq_payments')->where('enquiry_id',session()->get('unique_id_sess'))
// 08-09-2021
->where('razorpay_payment_link_reference_id',null)
->where('status',1)
->get();
?>
...
<div class="passport" id="passport">
    ...
    <hr>
    <div style="text-align: right">
                    <button data-toggle="modal" data-target ="#passport_edit" class="edit-button-design" > Edit</button> 
                    </div>
...
<tr>
    ...
    <td>
        <a href="{{url("public/uploads/image/passport/".$get_passport[0]->enquiry_id.'/'.$get_passport[0]->id_image)}}" target="_blank">
             <!--<img style="width:80px;"  src={{url("public/uploads/image/passport/".$get_passport[0]->id_image.'/'.$get_passport[0]->id_image)}}>-->
...
<tr>
    ...
    <td>
   {{--     {{$get_passport[0]->id_image?<a href="{{url("public/uploads/image/passport/".$get_passport[0]->enquiry_id.'/'.$get_passport[0]->id_image)}}" target="_blank">{{$get_passport[0]->id_image}}:''</a> '}} --}}
        
        <a href="{{url('public/uploads/image/passport/'.$get_passport[0]->enquiry_id.'/'.($get_passport[0]->passport_back_image?$get_passport[0]->passport_back_image:'-'))}}" target="_blank">
...
<div class="tab-content content-style  my-4" >
    ...
    <div role="tabpane" class="tab-pane " id="id_prf{{$e->id}}">
        ...
        <div style="text-align: right;">
            ...
            {{Form::open(array('url'=>"delete-other-id-proof", "method"=>"post",'style' => "display: initial;"))}}
        ...
        <input type="hidden" name="id_proof" value="{{$e->id_proof}}">
                <button type="submit" class="delete-button-design">Delete</button>
            {{Form::close()}}
        </div>
...
<p>  
    ...
    <a href="{{url('public/uploads/image/passport/other/'.$e->enquiry_id.'/'.$e->id_image)}}" target="_blank">  {!! $e->id_image !!}  
    <!--<img style="width:80px;"  src="{{url('public/uploads/image/passport/other/'.$e->enquiry_id.'/'.$e->id_image)}}" > -->
    </a> 
...
<tr><td>
    ...
    {{Form::close()}}
        ...
        <div role="tabpane" class="tab-pane " id="application">
            <table>
                ...
                <tbody>
                    ...
                    ->select('enquiries.name as name','sort_list_courses.id' ,'courses.course_name', 'courses.courseExpDate','courses.unique_id as course_code','courses.level','courses.intake','my_institutes.country','my_institutes.city','my_institutes.name as institute','sort_list_courses.priority','sort_list_courses.date',
                    'sort_list_courses.id as sort_list_id'
                    )
                    ...
                    <td>
                    <select style="width:50px" name="priority" class="jsjs" course-id="{{$item->course_code}}">
                        ...
                        <option value="{{$a}}" <?php echo (!empty($item->priority) && ($a == $item->priority)) ? 'selected' : ''; ?>>{{$a}}</option>;
                        <!--<option value="{{$a}}">{{$a}}</option>-->
                        @endfor
                    </select>
                    </td>
                    <td> 
                    <!--<input type="hidden" value="{{$item->course_code}}"> -->
                    <input type="date" class="CourseExpiryDate" value="{{$item->courseExpDate}}" courseId="{{$item->course_code}}"></td>
                    <td>
                        <a class="fa fa-trash" onclick="return confirm('Do you want to delete it?')" href="{{url(Request::segment(1).'/'.'application/application-delete/'.$item->sort_list_id)}}" ></a>
                        <a class="fa fa-edit" href="{{url(Request::segment(1).'/'.'application/application-process/'.$item->course_code .'/'.$item->sort_list_id)}}"></a>
                    </td>
                </tr>
                @endforeach
                </tbody>
        ...
        width: 22%;
        text-align: center;
        color: #fff;" href="{{url(Request::segment(1).'/'.'filter-course-section/?id='.session()->get('unique_id_sess'))}}">Create Application</a>
        </div>
...
$(document).ready(function(){
  ...
  var url = "{{url(Request::segment(1).'/'.'update_activity')}}?a="+unique_id+"&b="+id;
...
<script>
    ...
    function getState(val) {
        ...
        // console.log(data);
            $('.state-list').empty();
            $('.city-list').empty();
            $('.state-list').append('<option value="">Select state</option>')
            $('.city-list').append('<option value="">Select City</option>')
...
<script>
    // $(document).ready(function(){
    $(".CourseExpiryDate").change(function(){
        
        var conformation = confirm("Are you want to change the furture date");
        if(conformation){
        var CourseExpiryDatevalue = $(this).val();
        var course_unique_id = $(this).attr("courseId");
        // alert(course_unique_id);
        // console.log(CourseExpiryDatevalue);
       ...
       // data: {"CourseExpiryDatevalue": "khemchand"},
            $.ajax({
              url: "{{url(Request::segment(1)).'/'.'CourseExpiryUpdate'}}",
              method: "POST",
              data: {id: CourseExpiryDatevalue, courseId: course_unique_id},
              success: function (data) {
